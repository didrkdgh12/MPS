...
int areIdentical(node_t* a, node_t* b) {
    //Base Cases
    if(a == NULL && b == NULL) {
        return 1;
    }
    ...
    //Check if the data of both nodes is the same and check if their children are thesame
    return (a->module == b->module && a->cutline == b->cutline && areIdentical(a->left, b->left) && areIdentical(a->right, b->right));
}
...
int is_in_subtree(node_t* a, node_t* b) {
    ...
    //Check if the trees are identical at the current node
    if(areIdentical(a, b)) {
        return 1;
    }

    //Check if b is found in either the left or right child
...
void swap_topology(node_t* a, node_t* b) {
  ...
  // create a temp node, and hold on to the original parents so that we can link up the tree at the end properly
  node_t * temp =  a;
  node_t * aParent = a->parent;
  node_t * bParent = b->parent;
 
  // check to see which side A is currently on, and then swap B in for that
  if (a->parent->left == a) {
    // a is on the left
    aParent->left = b;
    b->parent = aParent;
  ...
  // check to see which side B is currently on, and then swap A (really temp now) in for that
  if (bParent->left == b) {
    // b is on the left
    bParent->left = temp;
    temp->parent = bParent;
...
//     node_t * cutNodePar = (node_t*)malloc(sizeof(node_t)); //this line tho

//     if(par == NULL) {
//         cutNodePar->parent = NULL;
//         cutNodePar->module = NULL;
//         cutNodePar->cutline = V;
//         par = cutNodePar;
//     }
...
//         node_t * moduleNode2 = (node_t*)malloc(sizeof(node_t));
//         par->left = moduleNode;
//         par->right = moduleNode2;
//         moduleNode->module = &modules[n+1];
//         moduleNode2->module = &modules[n];
//         moduleNode->left = NULL;
//         moduleNode->right = NULL;
//         moduleNode2->left = NULL;
//         moduleNode2->right = NULL;
//         moduleNode->cutline = UNDEFINED_CUTLINE;
//         moduleNode2->cutline = UNDEFINED_CUTLINE;
//         moduleNode2->parent = par;
...
//     //set links
//     par->left = cutNode;
//     par->right = moduleNode;

//     //set cutNode values
//     cutNode->module = NULL;
//     cutNode->cutline = V;

//     //set moduleNode values
...
// //    return NULL;
    ...
    node_t * moduleNode = (node_t *)malloc(sizeof(node_t));
    moduleNode->cutline = V;
    moduleNode->parent = par;
    moduleNode->module = NULL;
    
    //the right child (module)
    node_t * cutNode  = (node_t *)malloc(sizeof(node_t)); 
    cutNode->cutline = UNDEFINED_CUTLINE;
    cutNode->module = &modules[n];
    cutNode->parent = moduleNode;
    cutNode->left = NULL;
    cutNode->right = NULL;
    
    //setting the right and left of the node
    moduleNode->right = cutNode; 
    //recurse
    moduleNode->left = init_slicing_tree(moduleNode, n+1);
