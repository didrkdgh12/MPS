...
animation filler::fill( PNG & img, int x, int y,
        ...
        int w = img.width();
        int h = img.height();
        bool ** isProcessed = new bool*[img.width()];
        for(int i =0; i < w; i++) {
                isProcessed[i] = new bool[img.height()];
        }
        for(int i = 0; i < w; i++) {
                for(int j = 0; j < h; j++) {
        ...
        OrderingStructure <int> structX;
        OrderingStructure <int> structY;
        int oRc = img(x,y)->red;
        int oGc = img(x,y)->green;
        int oBc = img(x,y)->blue;
        int numberProcessed = 0;
        structX.add(x);
        structY.add(y);

        while(!structX.isEmpty() && !structY.isEmpty()) {
                int tempX = structX.remove();
                int tempY = structY.remove();
                int tRc = img(tempX, tempY)->red;
                int tGc = img(tempX, tempY)->green;
                int tBc = img(tempX, tempY)->blue;
                // int distance = pow((Rc - oRc), 2) + pow((Gc - oGc), 2) + pow((Bc - oBc), 2);
                int dist = (pow((tRc-oRc),2) + pow((tGc - oGc),2) + pow((tBc - oBc),2));
                cout << "Distance: " << dist << " tolerance: " << tolerance << endl;
                if(dist <= tolerance && !isProcessed[tempX][tempY]) {
                        cout << "Made it here" << endl;
                        *img(tempX, tempY) = fillColor(tempX, tempY);
                        isProcessed[tempX][tempY] = true;
                        if(tempX + 1 < w) {
                                structX.add(tempX + 1);
                                structY.add(tempY);
                        }
                        ...
                        numberProcessed++;
                        cout << numberProcessed << endl;
                        if(numberProcessed % frameFreq == 0) {
                                returnAnimation.addFrame(img);
                                cout << "Added Frame" << endl;
                        }
                }
        }
        for(int i = 0; i < w; i++) {
                delete [] isProcessed[i];
        }
        delete [] isProcessed;
