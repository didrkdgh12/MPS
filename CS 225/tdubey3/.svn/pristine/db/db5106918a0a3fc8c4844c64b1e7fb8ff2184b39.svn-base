...
{
	RGBAPixel color;
	int dist = abs(centX - x) + abs(centY - y);
    if(dist >= rad) {
        return color2;
    } else {
            color.red = color1.red - floor((dist*color1.red)/rad) + floor((dist*color2.red)/rad);
            color.blue = color1.blue - floor((dist*color1.blue)/rad) + floor((dist*color2.blue)/rad);
            color.green = color1.green - floor((dist*color1.green)/rad) + floor((dist*color2.green)/rad);
    }
    return color;
}
