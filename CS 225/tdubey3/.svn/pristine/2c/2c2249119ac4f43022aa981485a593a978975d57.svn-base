...
void BTree<K, V>::split_child(BTreeNode* parent, size_t child_idx)
{
     ...
     * E.g.
     * | 2 | 4 |
     * Mid eleroot->ment is at index (2 - 1) / 2 = 0 .
	...
	//parent->children[mid_child_idx] = new_right;
	//new_right->elements.assign(mid_elem_itr + 1, child->elements.end());
	//Left Child of the middle key handled
	//child->elements.erase(mid_elem_itr, child->elements.end());
	
	//Right Child of the middle key handled
	/*if(!child->is_leaf) {
...
void BTree<K, V>::insert(BTreeNode* subroot, const DataPair& pair)
{
	...
	auto insertion_pos_iter = subroot->elements.begin() + first_larger_idx; 
	//V temp = find(pair.key);
	if(subroot->is_leaf)
	{
		/*if(temp == V() || first_larger_idx > order)
		{
			return;
		}*/
		...
		subroot->elements.insert(insertion_pos_iter, pair);
		return;
	}
	else
	{
