...
PNG brighten(PNG original, int amount)
{
    ...
    for (size_t yi = 0; yi < original.height(); yi++)
    {
        ...
        {
            RGBAPixel currentPixel = *original(xi, yi);
            if (currentPixel.red + amount > 255) {
                currentPixel.red = 255;
            } else {
                currentPixel.red = currentPixel.red + amount;
            }

            if (currentPixel.green + amount > 255) {
                currentPixel.green = 255;
            } else {
                currentPixel.green = currentPixel.green + amount;
            }

            if (currentPixel.blue + amount > 255) {
                currentPixel.blue = 255;
            } else {
                currentPixel.blue = currentPixel.blue + amount;
            }
            *original(xi,yi) = currentPixel;
        }
    }
    return original;
}
...
PNG blendImages(PNG firstImage, PNG secondImage)
{
    for(size_t yi = 0; yi < firstImage.height(); yi++) {
            for(size_t xi = 0; xi < firstImage.width(); xi++) {
                RGBAPixel imgOnePix = *firstImage(xi, yi);
                RGBAPixel imgTwoPix = *secondImage(xi, yi);
                imgOnePix.red = (imgOnePix.red + imgTwoPix.red)/2;
                imgOnePix.green = (imgOnePix.green + imgTwoPix.green)/2;
                imgOnePix.blue = (imgOnePix.blue + imgTwoPix.blue)/2;
                *firstImage(xi, yi) = imgOnePix;
            }
    }
    return firstImage;
}
